
const images = {
  splash: require('./images/onboarding/splash.png'),
  app_text_logo: require('./images/onboarding/app_text_logo.png'),
  onboarding_images: [
    require('./images/onboarding/onboarding1.png'),
    require('./images/onboarding/onboarding2.png'),
    require('./images/onboarding/onboarding3.png')
  ],
  social: {
    google: require('./images/social-icons/google.png'),
    facebook: require('./images/social-icons/facebook.png'),
    twitter: require('./images/social-icons/twitter.png'),
  },
  icons: {
    back: require('./images/icons/back.png'),
    uploadIcon: require('./images/icons/upload.png'),
    marked: require('./images/icons/marked.png'),
    unmarked: require('./images/icons/unmarked.png'),
    chat: require('./images/icons/chat.png'),
    chat_outlined: require('./images/icons/chat-outlined.png'),
  },
  samples: {
    featured: require('./images/main/featured_sample.png'),
    category: require('./images/main/category_sample.png'),
    item: require('./images/main/item_sample.png'),
    profile_back: require('./images/main/profileback_sample.png'),
    avatar1: require('./images/main/avatar_sample1.png'),
    avatar2: require('./images/main/avatar_sample2.png'),
    listing: require('./images/main/listing_sample.png'),
  },
};

export default {
  images,
}